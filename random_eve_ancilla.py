...
def run_random_eve_ancilla(eve_strength):
    ...
    eve_axis_choice = random.choice(['z', 'x', 'y'])
    ...
    #Rotate Bob's Qubit so that Eve Favors the Random Axis That is Chosen
    if eve_axis_choice == 'x':
        circuit.append(cirq.ry(np.pi/2).on(qubits[1]))
    elif eve_axis_choice == 'y':
